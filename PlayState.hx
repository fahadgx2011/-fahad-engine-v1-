...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		...
		['WOW amazing!', 0.9], //From 80% to 89%
		['WOW chapter!', 1], //From 90% to 99%
		['WOW a so beautiful!!', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	override public function create()
	{
		...
		noteGroup.add(healthBar);

		iconP1 = new HealthIcon(boyfriend.healthIcon, true);
		iconP1.y = healthBar.y - 75;
		iconP1.visible = !ClientPrefs.data.hideHud;
		iconP1.alpha = ClientPrefs.data.healthBarAlpha;
		noteGroup.add(iconP1);

		iconP2 = new HealthIcon(dad.healthIcon, false);
		iconP2.y = healthBar.y - 75;
		iconP2.visible = !ClientPrefs.data.hideHud;
		iconP2.alpha = ClientPrefs.data.healthBarAlpha;
		noteGroup.add(iconP2);
		...
		updateScore(true);
		noteGroup.add(scoreTxt);

		botplayTxt = new FlxText(400, healthBar.y - 90, FlxG.width - 800, Language.getPhrase("chapter").toUpperCase(), 32);
		botplayTxt.setFormat(Paths.font("vcr.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		botplayTxt.scrollFactor.set();
		botplayTxt.borderSize = 1.25;
		botplayTxt.visible = cpuControlled;
		noteGroup.add(botplayTxt);
	...
	public dynamic function updateScoreText()
	{
		...
		if(!instakillOnMiss) tempScore = Language.getPhrase('score_text', 'Bambi Score: {1} | fahad Breaks: {2} | Accuracy: {3}', [songScore, songMisses, str]);
		else tempScore = Language.getPhrase('score_text_instakill', 'Bambi Score: {1} | Accuracy: {2}', [songScore, str]);
	...
	var iconsAnimations:Bool = false;
	function set_health(value:Float):Float // You can alter how icon animations work here
	{
		...
		// update health bar
		health = value;
		var newPercent:Null<Float> = FlxMath.remapToRange(FlxMath.bound(healthBar.valueFunction(), healthBar.bounds.min, healthBar.bounds.max), healthBar.bounds.min, healthBar.bounds.max, 35, 100);
		healthBar.percent = (newPercent != null ? newPercent : 35);
